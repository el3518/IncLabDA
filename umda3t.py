...
def obtain_label(loader, netF, netB, netC, args):
    start_test = True #loader = dset_loaders_ini['test']
    with torch.no_grad():
        iter_test = iter(loader)
    ...
    '''
    #proto = src_anc
    sim = torch.matmul(torch.from_numpy(proto), torch.from_numpy(initc).transpose(0,1))
    s_idx = sim.argmax(dim=1)
    t_idx = sim.argmax(dim=0)
    
    sim = cdist(initc, proto, args.distance)
    cen_label = sim.argmin(axis=1)
    '''
...
def train_target(args):
    ...
    while iter_num < max_iter:
        ...
        if iter_num % interval_iter == 0:
            ...
            #mem_label, ENT_THRESHOLD, labelset = obtain_label(dset_loaders_ini['test'], netF, netB, netC, args, src_anc)
        ...
        total_loss = classifier_loss  + uk_loss  + con_loss + cen_loss #+ k_loss 
...
if __name__ == "__main__":
    ...
    parser.add_argument('--output_src', type=str, default='ood')
