...
def data_load_src(args, source_tra, source_tst): 
    ## prepare data
    #source_tra = args.s1_dset_path
    dsets = {}
    dset_loaders = {}
    train_bs = args.batch_size
    tra_src = open(source_tra).readlines()    
    tst_src = open(source_tst).readlines()
    ...
    if not args.da == 'uda':
        ...
        new_src = []
        for i in range(len(tra_src)):
            rec = tra_src[i]
            reci = rec.strip().split(' ')
            if int(reci[1]) in args.src_classes:
                line = reci[0] + ' ' + str(label_map_s[int(reci[1])]) + '\n'   
                new_src.append(line)
        tra_src = new_src.copy()
        ...
        for i in range(len(tst_src)):
            ...
            if int(reci[1]) in args.src_classes:
                line = reci[0] + ' ' + str(label_map_s[int(reci[1])]) + '\n'   
                new_src_tst.append(line)
        tst_src = new_src_tst.copy()
    ...
    dsets["source_tr"] = ImageList(tra_src, transform=image_train())
    dset_loaders["source_tr"] = DataLoader(dsets["source_tr"], batch_size=train_bs, shuffle=True, num_workers=args.worker, drop_last=False)
    dsets["source_te"] = ImageList(tst_src, transform=image_test())
...
def train_source(args):
    ...
    dset_loaders_s1 = data_load_src(args, args.s1_dset_path, args.s1t_dset_path)
    dset_loaders_s2 = data_load_src(args, args.s2_dset_path, args.s2t_dset_path)
    ...
    acc_init = 0
    count = 0
    max_iter = args.max_epoch * max(len(dset_loaders_s1["source_tr"]),len(dset_loaders_s2["source_tr"]))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dset', type=str, default='DomainNet', choices=['office31', 'DomainNet'])
    ...
    args.sk = 2
    folder = './dataset/'
    args.s1_dset_path = folder + args.dset + '/' + names[task[args.s1]] + '_tra.txt'
    args.s2_dset_path = folder + args.dset + '/' + names[task[args.s2]] + '_tra.txt'
    
    args.s1t_dset_path = folder + args.dset + '/' + names[task[args.s1]] + '_tst.txt'
    args.s2t_dset_path = folder + args.dset + '/' + names[task[args.s2]] + '_tst.txt'
   
    args.test_dset_path = folder + args.dset + '/' + names[task[args.t]] + '_tst.txt'     

    if args.dset == 'office31':
        if args.da == 'poda':
            args.class_num = 20
            args.class_all = 31
            #args.class_share = [i for i in range(10)] #listu
            #args.src_private = [i for i in range(10,20)]
            #args.tar_private = [i for i in range(20,31)]            
            ...
            #args.class_share = ['back_pack', 'calculator', 'keyboard', 'monitor', 'mouse', 'mug', 'bike', 'laptop_computer', 'headphones', 'projector']
            #args.src_private = ['bike_helmet', 'bookcase', 'bottle', 'desk_chair', 'desk_lamp', 'desktop_computer', 'file_cabinet', 'letter_tray', 'mobile_phone', 'paper_notebook']
            #args.tar_private = ['pen', 'phone', 'printer', 'punchers', 'ring_binder', 'ruler', 'scissors', 'speaker', 'stapler', 'tape_dispenser', 'trash_can']
    ...
    if args.dset == 'DomainNet':
        if args.da == 'poda':
            args.class_num = 200
            args.class_all = 345
            args.class_share = [i for i in range(150)]
            args.src_private = [i for i in range(150,200)]
            args.tar_private = [i for i in range(200,345)]
            args.src_classes = list(set(args.class_share)|set(args.src_private))
            args.tar_classes = list(set(args.class_share)|set(args.tar_private))

    
    traepo = 'sonly-mix-com'
