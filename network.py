...
class MDAdomnet(nn.Module):
    ...
    def __init__(self, num_domain=2):
        super(MDAdomnet, self).__init__()        
        
        self.domain = nn.Sequential()
        self.domain.add_module('fc1', nn.Linear(256, 1024))
        self.domain.add_module('relu1', nn.ReLU(True))
        self.domain.add_module('dpt1', nn.Dropout())
        self.domain.add_module('fc2', nn.Linear(1024, 1024))
        self.domain.add_module('relu2', nn.ReLU(True))
        self.domain.add_module('dpt2', nn.Dropout())
        self.domain.add_module('fc3', nn.Linear(1024, num_domain))

    def forward(self, x):
        x = self.domain(x)
        return x
#######################################################################################
class proto_gen(nn.Module):
    def __init__(self, n=10):
        super(proto_gen, self).__init__()        
        self.n = n
        self.fcs = nn.Sequential()
        self.fc = {}
        for i in range(n):
