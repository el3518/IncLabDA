...
def train_target(args):
    ...
    dset_loaders_tst = data_load_tst(args)
    ...
    tt = 0
    iter_num = 0
    max_iter = args.max_epoch * int(len(dset_loaders_ini["target"])/3)
    ...
    while iter_num < max_iter:
        ...
        if iter_num % interval_iter == 0 or iter_num == max_iter:
            netF.eval()
            netB.eval()
            acc_os1, acc_os2, acc_unknown, hm = cal_acc_oda(dset_loaders_tst['test'], netF, netB, netC, True, ENT_THRESHOLD)            
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dset', type=str, default='DomainNet', choices=['VISDA-C', 'office31', 'OfficeHome', 'offcal', 'CLEF', 'DomainNet'])
    ...
    args.t_dset_path = folder + args.dset + '/' + names[task[args.t]] + '_tra.txt'
    ...
    traepo = 1
    save_tag = 'epo_tar-com' + str(traepo)
