...
def train_source(args):
    ...
    while iter_num < max_iter:
        ...
        src_cen = clu_anc_para(netC, netD, args)   
...
def clu_anc_para(netC, netD, args):#netC=netC1
    para=[]
    for k,v in netC.named_parameters():
        #print(k)
    ...
    for k,v in netD.named_parameters():
        ...
        if k[last] =='3':
            #print(v.shape)
            parad.append(v.cpu().detach())
    paradd = torch.Tensor(len(parad),1,para[0].shape[1])
    parad = torch.cat(parad, out=paradd)
    ...
    _, predict = torch.max(output, 1) parad.type
    '''
                
    #anc = np.zeros([para[0].shape[0], para[0].shape[1]+1])

    ancd = norm_fea(parad).numpy()
    anc = ancd
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dset', type=str, default='office31', choices=['office31', 'DomainNet'])
