...
def data_load_src_h(args, source_name, source_class): 
    ## prepare data
    #source_name = args.s1_dset_path
    dsets = {}
    dset_loaders = {}
...
def discriminator_loss(net, fea, lab, args):   #lab=Slabels net=netD
    bin_label = F.one_hot(lab, args.class_num) #fea=netB(images)
    pre_label = net(fea)   #pre_label=netB(netF(inputs_source1)), lab=labels_source1
    cls_loss = BCELossForMultiClassification(bin_label, pre_label)
    return cls_loss

def discriminator_loss_h(net, fea, lab, args, source_class):   #lab=Slabels net=netD
    bin_label = F.one_hot(lab, args.class_num) #fea=netB(images)
    pre_label = net(fea)   #fea=netB(netF(inputs_source1)), lab=labels_source1
    label_map_s = {}
    for i in range(len(args.src_classes)):
        label_map_s[args.src_classes[i]] = i
    cen_idx = []
    for i in range(len(source_class)):#source_class=args.src_classes1
        cen_idx.extend([label_map_s[args.src_classes1[i]]])
    
    cls_loss = BCELossForMultiClassification(bin_label[:,cen_idx], pre_label[:,cen_idx])
    return cls_loss
...
def train_source(args):
    ...
    dset_loaders_s1 = data_load_src_h(args, args.s1_dset_path, args.src_classes1)
    dset_loaders_s2 = data_load_src_h(args, args.s2_dset_path, args.src_classes2)
    dset_loaders_s3 = data_load_src_h(args, args.s3_dset_path, args.src_classes3)
    ...
    while iter_num < max_iter:
        ...
        classifier_loss = discriminator_loss_h(netD, netB(netF(inputs_source1)), labels_source1, args, args.src_classes1)
        ...
        classifier_loss = discriminator_loss_h(netD, netB(netF(inputs_source2)), labels_source2, args, args.src_classes2)
        ...
        classifier_loss = discriminator_loss_h(netD, netB(netF(inputs_source1)), labels_source1, args, args.src_classes3)
        ...
        cen_idx = []
        for i in range(len(args.src_classes1)):#i=1
            cen_idx.extend([label_map_s[args.src_classes1[i]]])
        
        cen_loss = np.linalg.norm(src_anc[cen_idx]-src_cen1[cen_idx], ord=2, keepdims=True) 
        
        cen_idx = []
        for i in range(len(args.src_classes2)):#i=1
            cen_idx.extend([label_map_s[args.src_classes2[i]]])
        
        cen_loss += np.linalg.norm(src_anc[cen_idx]-src_cen2[cen_idx], ord=2, keepdims=True) 
        
        cen_idx = []
        for i in range(len(args.src_classes3)):#i=1
            cen_idx.extend([label_map_s[args.src_classes3[i]]])
        
        cen_loss += np.linalg.norm(src_anc[cen_idx]-src_cen3[cen_idx], ord=2, keepdims=True)
...
def test_target(args):
    ...
    if args.da == 'oda':
        acc_os1, acc_os2, acc_unknown = cal_acc_oda(dset_loaders_t['test'], netF, netB, netC)
...
if __name__ == "__main__":
    ...
    if args.dset == 'OfficeHome':
        if args.da == 'poda':
            ...
            #args.class_share1 = [0,1,2,3,4,5]
            #args.class_share2 = [2,3,4,5,6,7]
            #args.class_share3 = [4,5,6,7,8,9]
            #args.class_share = list(set(args.class_share1)|set(args.class_share2)|set(args.class_share3))            
            args.class_share = [i for i in range(10)]
            args.src_private1 = [i for i in range(10,15)]
            args.src_private2 = [i for i in range(15,20)]
            args.src_private3 = [i for i in range(20,25)]
            args.tar_private = [i for i in range(25,65)]
            args.src_classes = list(set(args.class_share)|set(args.src_private1)|set(args.src_private2)|set(args.src_private3))
            '''
            ...
            args.src_classes3 = list(set(args.class_share3)|set(args.src_private3))            
            
            '''
            args.src_classes1 = list(set(args.class_share)|set(args.src_private1))
            args.src_classes2 = list(set(args.class_share)|set(args.src_private2))
            args.src_classes3 = list(set(args.class_share)|set(args.src_private3))            
            #'''
            args.tar_classes = list(set(args.class_share)|set(args.tar_private))


    traepo = 'sonly-mix-comh-g'
